...
export function createFhirReferences(strReferences) {
    if (!strReferences.length || strReferences.length < 1)
        return [];
    let fhirReferences = [];
    strReferences.forEach(function (reference) {
        let fhirReference = createFhirReference(reference);
        if (fhirReference.reference && fhirReference.reference != "")
            fhirReferences.push(fhirReference);
    });
    return fhirReferences;
}
export function createFhirReference(strReference, type, display) {
    if (strReference == "")
