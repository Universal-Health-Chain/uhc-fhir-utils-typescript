...
export function optionsVaccinationProcedureCovid19SNOMED(globalOrRegional) {
    if (!globalOrRegional)
        return;
...
function getIndexCodesByCategoriesSNOMED(customCategory, subCategory, parentGroupCode) {
    let codes = [];
    const groupCodes = Object.keys(GlobalIndexSNOMED.Category[customCategory][subCategory][parentGroupCode]);
...
export function translateCodesSNOMED(codes, labelKeySection) {
