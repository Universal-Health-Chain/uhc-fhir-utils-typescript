...
export function createCodeableConceptWithOptionalLanguage(typeCode, codeSystem, customLanguageFile, systemVersion, userSelected) {
    if (!typeCode)
        return [];
...
export function createCodeableConceptsArrayOfSystem(inputCodes, system, customLanguageFile) {
    let output = [];
...
export function createCodingArrayOfSystem(inputCodes, codeSystem, englishData) {
    let output = [];
    if (!inputCodes || inputCodes.length || inputCodes.length < 1)
        return output; // throw new Error ("Missing codes")
