...
class CommonFHIR {
    constructor() {
        ...
        this.getCleanId = (data) => getCleanId(data);
...
*/
function getCleanId(data) {
    try {
        if (data && data.id)
            return cleanIdWithoutReferenceFHIROrURN(data.id) || ""; // it can be an URN with the ID at the end of the string
        else
            return "";
    }
    catch (e) {
