...
// TODO: urn identifier
export function createDocumentReferenceToCertificate(documentIdentifier, // universal ID of the document (UUID v4 format), it can have several certified / verifiable versions (VC)
...
documentLanguage, documentTitle, documentDescription, contentTitle, contentFormatURN // URN format defined by IHE or HL7
    ...
    try {
        let fhirAttachment = createFhirAttachment(mimeType, undefined, undefined, undefined, undefined, undefined, undefined, certifiedBytesAsBase64, undefined);
        let fhirContent = { attachment: fhirAttachment };
        if (!!contentFormatURN) {
            const coding = createCoding(contentFormatURN, "http://ihe.net/fhir/ihe.formatcode.fhir/ValueSet/formatcode");
...
export function createDocumentReference(contents, categoryCodeLOINC, id, identifiers) {
    if (!id)
        id = uuidRandom();
