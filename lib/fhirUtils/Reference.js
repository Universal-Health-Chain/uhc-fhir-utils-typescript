...
export class Reference {
    ...
    getReferencesByIdentifierAndSystem(referencesFHIR, system) {
        return getReferencesByIdentifierAndSystem(referencesFHIR, system);
    }
    getLiteralReferencesStrings(referencesFHIR) {
        return getLiteralReferencesStrings(referencesFHIR);
    }
    createReferenceFHIR(strReference, type, display) {
        return createReferenceFHIR(strReference, type, display);
    }
    createReferencesByLiteralURLs(literalReferences) {
        return createReferencesByLiteralURLs(literalReferences);
    }
    createReferenceIdentifiers(identifiers, system) {
...
export function getLiteralReferencesStrings(referencesFHIR) {
    if (!referencesFHIR.length || referencesFHIR.length < 1)
        throw new Error("Missing FHIR References");
...
export function createReferenceFHIR(strReference, type, display) {
    if (strReference == "")
...
export function createReferencesByLiteralURLs(literalReferences) {
