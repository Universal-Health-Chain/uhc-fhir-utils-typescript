...
class Bundle {
    ...
    addResourcesWithOptions(bundle, resources, sectionCodeLOINC, excludeResources, fullUrlPrefix) {
    ...
    createBundleDocumentAndCompositionWithIds(bundleId, compositionId, authorType, date, title, status, typeDocumentCode, typeDocumentSystem, typeDocumentDisplay, authorId, authorURI, authorDisplay, language, resources, excludeResources) {
        return createBundleDocumentAndCompositionWithIds(bundleId, compositionId, authorType, date, title, status, typeDocumentCode, typeDocumentSystem, typeDocumentDisplay, authorId, authorURI, authorDisplay, language, resources, excludeResources);
    }
} // end class Bundle
...
/** Creates a Bundle document with all mandatory properties in the document 'Composition' resource (the index) */
function createBundleDocumentAndCompositionWithIds(bundleId, compositionId, authorType, date, title, status, typeDocumentCode, typeDocumentSystem, typeDocumentDisplay, authorId, authorURI, authorDisplay, language, resources, excludeResources) {
    // console.log(`createBundleDocumentWithCompositionAndURNs with ${resources.length} resources`)
    let basicComposition = Composition_1.createCompositionWithIdAndAuthor(compositionId, authorType, date, title, status, typeDocumentCode, typeDocumentSystem, typeDocumentDisplay, authorId, authorURI, authorDisplay, language);
...
function getResourceWithOptionalMetaData(fhirResource, fhirBundle, defaultSectionLOINC, defaultServiceType) {
    ...
    if (loincSectionCode !== "") {
        fhirResource.meta = Object.assign(Object.assign({}, fhirResource.meta), { section: loincSectionCode });
    }
...
function addReferencesInBundleEntriesToComposition(composition, entries, loincSectionCode) {
    ...
    let newComposition = Object.assign({}, composition);
...
function addResourceAsBundleEntry(bundle, resource, fullUrl) {
    if (!resource.resourceType || !resource.id) {
        console.log(`skipping not valid resource`);
        return bundle;
    }
    ...
    if (existingResourcesIds.includes(resource.id)) {
        console.log(`skipping already present resource ID ${resource.id}`);
        return bundle;
    }
    ...
    updatedBundle.entry ? updatedBundle.entry.push(newEntry) : updatedBundle.entry = [newEntry];
...
function addResourcesToSection(bundleDocument, resources, sectionCode, excludeResources, fullUrlPrefix) {
    ...
    // Then it adds the new resources to the bundle. Copying original object is less efficient but avoid problems on tests
    const updatedBundle = addResourcesWithOptions(bundleDocument, resources, undefined, excludeResources, fullUrlPrefix);
    // Finally it replaces the composition. Copying original object is less efficient but avoid problems on tests
    return Composition_1.updateComposition(updatedBundle, updatedComposition); // instead of replaceResourceById(newComposition, newBundle)
}
...
function addResourceToSection(bundleDocument, resource, sectionCode, fullUrl) {
    let updatedBundle = bundleDocument; // less efficient but avoid problems on tests
    if (!updatedBundle || !updatedBundle.entry || !updatedBundle.entry.length
        || updatedBundle.entry.length < 1 || !updatedBundle.entry[0].resource
        || !updatedBundle.entry[0].resource.resourceType
        || updatedBundle.entry[0].resource.resourceType !== 'Composition') {
        throw new Error(`Bundle document does not have Composition`);
    }
    else {
        // adding resource to the composition and updating the composition in the bundle document
        const composition = Composition_1.addResourcesToComposition(updatedBundle.entry[0].resource, [resource], sectionCode);
        updatedBundle.entry[0].resource = composition; // simple operation, it is not necessary to call to 'updateComposition'
        // adding the resource as bundle entry
        updatedBundle = addResourceAsBundleEntry(updatedBundle, resource, fullUrl);
        return updatedBundle;
    }
}
        ...
        if (!resource || !resource.resourceType || !resource.id) {
            ...
            if (!excludeResources ||
                ...
                if (existingResourcesIds.includes(resource.id)) {
                    ...
                    if (!sectionCodeLOINC) {
                        // less efficient but avoid problems on tests
                        updatedBundle = addResourceAsBundleEntry(bundle, resource);
                    }
                    else {
                        ...
                        // less efficient but avoid problems on tests
                        updatedBundle = addResourceToSection(bundle, resource, sectionCodeLOINC, fullUrl);
                    }
                }
            }
        } // if excluded resource type do nothing
    });
    return updatedBundle;
}
