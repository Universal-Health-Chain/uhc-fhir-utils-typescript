...
import { addResourcesToComposition, getSectionByCodeInComposition, createEmptyCompositionSection, addReferencesToCompositionSection, putSectionInComposition, createDefaultComposition } from "./Composition";
export class Bundle {
    constructor() {
    }
    ...
    createBundleDocumentWithComposition(resources, authorReferenceId, typeDocumentCodeLOINC) {
    ...
    getAllResourcesInBundleEntries(bundle) {
...
export function getTimestamp(fhirBundle) {
...
// TODO: ADD RESOURCES TO COMPOSITION SECTIONS AND FULLURL
// It adds a bundle resource including Composition or HeaderMessage and skips if already present
export function addResourceToBundle(bundle, resource, sectionCode, sectionSystem) {
...
// TODO: ADD RESOURCES TO COMPOSITION SECTIONS
// It adds resources except 'Composition', 'MessageHeader' and also skips if empty resource.id or already exists, both for Bundle Document and Bundle Message
export function addAdditionalResourcesToBundle(bundle, resources, sectionCode, sectionSystem) {
...
function addReferencesInBundleEntriesToComposition(composition, entries, sectionCode, sectionSystem) {
    ...
    if (!newComposition.section)
        ...
        section = getSectionByCodeInComposition(newComposition, sectionCode, sectionSystem); // error if section has no code
...
export function createBundleDocumentWithComposition(resources, authorReferenceId, typeDocumentCodeLOINC) {
    // console.log("createBundleDocumentWithComposition with resources = ", JSON.stringify(resources))
    let emptyComposition = createDefaultComposition(authorReferenceId, typeDocumentCodeLOINC);
...
// the first resource type in the bundle document must be a composition: http://hl7.org/fhir/bundle.html
export function isIPS(bundleDocument) {
    if (!bundleDocument || !bundleDocument.type || bundleDocument.type != R4.BundleTypeKind._document || !bundleDocument.entry || !bundleDocument.entry.length || !bundleDocument.entry[0]
        ...
        || bundleDocument.entry[0].resource.type.coding[0].code != GlobalIndexLOINC.Category.HealthSection.CompositionIPS)
        return false;
    else
        return true;
}
// TODO: set the UHC identifier, not only the id
export function createEmptyIPS(authorReferenceId) {
    let typeDocumentCodeLOINC = GlobalIndexLOINC.Category.HealthSection.CompositionIPS;
    return createBundleDocumentWithComposition(undefined, authorReferenceId, typeDocumentCodeLOINC);
}
// TODO: it does not check for the meta.version of the document
export function addEntriesToBundle(bundle, entries) {
...
// TODO: generate the entry.fullUrl and reference as "resourceType/id"
export function addResourcesBySection(bundleDocument, sectionCode, sectionSystem, resources) {
    ...
    let newComposition = addResourcesToComposition(compositions[0], resources, sectionCode, sectionSystem);
...
export function addEntriesBySection(bundleDocument, bundleEntries, sectionCode, sectionSystem) {
    let compositions = getResourcesByTypes(bundleDocument, ["Composition"]);
    if (compositions.length < 1)
...
export function getReferencesInSection(section) {
    // TODO: do validations
...
export function getResourcesInSection(bundleDocument, sectionCode, sectionSystem) {
    let compositions = getResourcesByTypes(bundleDocument, ["Composition"]);
    if (compositions.length < 1)
        return {}; //returns empty
    if (!compositions[0].section || compositions[0].section.length < 1)
        return {}; //returns empty
    let sectionInComposition = getSectionByCodeInComposition(compositions[0], sectionCode, sectionSystem);
...
export function getAllResourcesInBundleEntries(bundle) {
...
export function getAllResourcesWithoutCompositionOrMessageHeader(bundle) {
    if (!bundle || bundle.resourceType != "Bundle" || !bundle.entry || !bundle.entry.length || bundle.entry.length < 1)
        return []; // or error?
    let results = [];
    // let entries:R4.IBundle_Entry[] = bundle.entry as R4.IBundle_Entry[]
...
// It returns an arry of IDs, splitting the ID by "/" and getting the last string after the slash
export function getResourceIdsInBundle(bundle) {
...
export function getResourcesByTypes(bundle, resourceTypes) {
...
export function getResourceByIdInBundle(resourceId, bundle) {
...
export function getObservationsByCode(bundle, code) {
...
// It replaces the given resource in the right Bundle.entry without generating Bundle.entry[].fullUrl
export function replaceResourceById(resource, bundle) {
...
export function getMediaInBundle(bundle) {
    if (!bundle || bundle.resourceType != "Bundle")
        return [];
    if (!bundle.entry || bundle.entry.length <= 0)
        return [];
