...
class Bundle {
    constructor() {
        ...
        this.getCleanIdOfDocumentComposition = (fhirBundle) => getCleanIdOfDocumentComposition(fhirBundle);
    }
    ...
    addAdditionalResourcesToBundle(bundle, resources) {
...
function getCleanIdOfDocumentComposition(fhirBundle) {
    ...
    if (!fhirBundle || !fhirBundle.entry || !fhirBundle.entry.length || fhirBundle.entry.length < 1 ||
        ...
        return CommonFHIR_1.getCleanIdOfResource(fhirBundle.entry[0].resource);
    }
}
...
function getResourcesInSection(bundleDocument, loincSectionCode) {
    let compositions = getResourcesByTypes(bundleDocument, ["Composition"]);
    if (compositions.length < 1)
        return {}; //returns empty
