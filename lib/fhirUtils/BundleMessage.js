...
class BundleMessage {
    ...
    getAllResources(bundle) {
        return Bundle_1.getAllResourcesInBundleEntries(bundle);
    }
    getAllResourcesWithoutCompositionOrMessageHeader(bundle) {
        return Bundle_1.getAllResourcesWithoutCompositionOrMessageHeader(bundle);
    }
    ...
    getResourcesByTypes(bundle, resourceTypes) {
        return Bundle_1.getResourcesByTypes(bundle, resourceTypes);
    }
    ...
    getTagsInBundle(fhirBundle) {
        return Bundle_1.getTagsInBundleResource(fhirBundle);
    }
...
function getCleanIdOfMessageHeader(fhirBundle) {
    if (!fhirBundle || !fhirBundle.entry || !fhirBundle.entry.length || fhirBundle.entry.length < 1 ||
        ...
        return CommonFHIR_1.getCleanIdOfResource(fhirBundle.entry[0].resource.id);
    }
}
...
function createBasicBundleMessage(messageId, textMessage, attachments) {
    ...
    messageBundle = Bundle_1.addAdditionalResourcesToBundle(messageBundle, [messageHeader]);
...
function createBundleMessage(senderId, messageId, bundleDoc, textMessage, attachments, entityTitle, entityDescription) {
    ...
    if (bundleDoc) {
        ...
        BundleMessage = Bundle_1.addAdditionalResourcesToBundle(BundleMessage, [patientRecordAudit, bundleDoc]);
    }
...
function getBundleDocumentInBundleMessage(bundleMessage) {
    let bundleDocuments = Bundle_1.getResourcesByTypes(bundleMessage, ["Bundle"]);
...
function getTextInBundleMessage(fhirMessage) {
    let resources = Bundle_1.getResourcesByTypes(fhirMessage, ["Communication"]);
