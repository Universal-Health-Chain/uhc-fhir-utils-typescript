...
export function createBundleMessage(senderId, messageId, bundleDoc, textMessage, attachments, entityTitle, entityDescription) {
...
export function getBundleDocumentInBundleMessage(bundleMessage) {
    let bundleDocuments = getResourcesByTypes(bundleMessage, ["Bundle"]);
    if (bundleDocuments.length && bundleDocuments.length > 0) {
        return bundleDocuments[0];
    }
    return {};
}
export function addCommunicationTextToBundleMessage(fhirMessage, text) {
    ...
    let newMessage = addResourceToBundle(fhirMessage, newCommunication); // using addResourceToBundle instead of addAdditionalResourcesToBundle
    // //console.log("newMessage = ", JSON.stringify(newMessage))
    return newMessage;
}
export function getTextInBundleMessage(fhirMessage) {
    let resources = getResourcesByTypes(fhirMessage, ["Communication"]);
...
export function addAttachmentsToBundleMessage(fhirMessage, attachments) {
    ...
    // It adds the new communication to the bundle of the message and returns a valid bundle
    let newMessage = addResourceToBundle(fhirMessage, communication); // using addResourceToBundle instead of addAdditionalResourcesToBundle
    // //console.log("newMessage = ", JSON.stringify(newMessage))
    return newMessage;
}
export function getAttachmentsInBundleMessage(fhirMessage) {
    let resources = getResourcesByTypes(fhirMessage, ["Communication"]);
    if (resources.length < 1) {
        // //console.log(("No communication resource was found in message"))
        return {};
...
export function addAuditEventToBundleMessage(fhirMessage, auditEvent) {
