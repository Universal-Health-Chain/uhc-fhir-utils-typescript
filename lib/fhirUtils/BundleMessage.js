...
function createBasicBundleMessage(messageId, textMessage, attachments) {
    ...
    messageBundle = Bundle_1.addResourcesWithOptions(messageBundle, [messageHeader]);
...
function createBundleMessage(senderId, messageId, bundleDoc, textMessage, attachments, entityTitle, entityDescription) {
    ...
    if (bundleDoc) {
        ...
        let patientRecordAudit = AuditEvent_1.createPatientRecordAuditEvent(senderId, entityTitle, entityDescription);
        BundleMessage = Bundle_1.addResourcesWithOptions(BundleMessage, [patientRecordAudit, bundleDoc]);
    }
...
function addCommunicationTextToBundleMessage(fhirMessage, text) {
    ...
    let newMessage = Bundle_1.addResourceAsBundleEntry(fhirMessage, newCommunication); // using addResourceToBundle instead of addAdditionalResourcesToBundle
...
function addAttachmentsToBundleMessage(fhirMessage, attachments) {
    ...
    // It adds the new communication to the bundle of the message and returns a valid bundle
    let newMessage = Bundle_1.addResourceAsBundleEntry(fhirMessage, communication); // using addResourceToBundle instead of addAdditionalResourcesToBundle
    // //console.log("newMessage = ", JSON.stringify(newMessage))
    return newMessage;
}
...
function addAuditEventToBundleMessage(fhirMessage, auditEvent) {
    ...
    return Bundle_1.addResourceAsBundleEntry(fhirMessage, auditEvent); // using addResourceToBundle instead of addAdditionalResourcesToBundle
}
